...
struct Shader : IShader {
    ...
    virtual bool fragment(const vec3 bar, TGAColor &gl_FragColor) {
        ...
        double spec = std::pow(std::max(-r.z, 0.), 5+sampleBilinear2D(model.specular(), uv)[0]); // specular intensity, note that the camera lies on the z-axis (in view), therefore simple -r.z

        TGAColor c = sampleBilinear2D(model.diffuse(), uv);
        for (int i : {0,1,2})
...
int main(int argc, char** argv) {
    ...
    std::vector<double> zbuffer(width*height*4, std::numeric_limits<double>::max());
    std::vector<TGAColor> colorbuffer(width*height*4,TGAColor());
    ...
    for (int m=1; m<argc; m++) { // iterate through all input objects
        ...
        for (int i=0; i<model.nfaces(); i++) { // for every triangle
            ...
            triangle(clip_vert, shader, framebuffer, zbuffer, colorbuffer); // actual rasterization routine call
        }
    }
    for (int x = 0; x < width; x++) {
        for (int y = 0; y < height; y++) {
            for (int i = 0; i < 4; i++) {
                framebuffer.add(x, y, colorbuffer[4*(x+y*width)+i] * 0.25);
            }
        }
    }
    framebuffer.write_tga_file("SSAA.tga"); // the vertical flip is moved inside the function
    return 0;
}
