...
void TGAImage::add(const int x, const int y, const TGAColor &c) {
    if (!data.size() || x<0 || y<0 || x>=w || y>=h) return;
    data[(x+y*w)*bpp] += c.bgra[0];
    data[(x+y*w)*bpp+1] += c.bgra[1];
    data[(x+y*w)*bpp+2] += c.bgra[2];
    data[(x+y*w)*bpp+3] += c.bgra[3];
}
